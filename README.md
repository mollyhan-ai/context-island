...
当前版本用于产品与交互验证。公开原型页面：
[mollyhan-ai.github.io/context-island-demo](https://mollyhan-ai.github.io/context-island-demo/)。
...
当前共有 **75 项测试**，覆盖解析容错、义项校验、句子分段、固定语法标签、有限重试、
缺词处理、重复例句拦截、缓存、公开接口限流和 ECDICT 导入。

## 部署后端

仓库根目录的 `render.yaml` 可以创建一项 Render Web Service。默认使用免费测试实例，
部署时需要在 Render 的保密配置中填写 `LLM_API_KEY`，密钥不会写入仓库。

[在 Render 部署](https://render.com/deploy?repo=https://github.com/mollyhan-ai/context-island)

免费实例闲置后会休眠，首次唤醒可能需要约一分钟。公开配置按访客每小时 10 次、服务
整体每小时 60 次限制真实模型生成；相同单词、领域和难度的缓存结果不占生成额度。
